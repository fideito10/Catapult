...
def graficar_velocidad_sesion(velocidad_stats, tipo_sesion):
    ...
    fig.add_trace(go.Bar(
        ...
        y=velocidad_stats['Velocidad Alta Media (km)'],  # Usar km para consistencia
        name='Sprint Distance',
        marker_color='rgb(70, 70, 70)',
        text=velocidad_stats['Velocidad Alta Media (km)'].round(2),
        textposition='outside',
        hovertemplate='<b>%{x}</b><br>Sprint Distance: %{y:.2f} km<br>N° Jugadores: %{customdata[0]}<br>Total: %{customdata[1]:.2f} km<extra></extra>',
    ...
    fig.update_layout(
        title={
            'text': f'Sprint Distance (km) - {tipo_sesion}',
            'font': {'size': 24},
            'x': 0.5,
            'xanchor': 'center'
        },
...
def obtener_estadisticas_velocidad(sesiones_df):
    try:
        ...
        # Convertir las columnas a numéricas, manejando diferentes formatos
        for col in columnas_velocidad:
            if col in sesiones_df.columns:
                # Si los datos son cadenas, procesarlos antes de convertir
                if pd.api.types.is_object_dtype(sesiones_df[col]):
                    # Reemplazar puntos como separadores de miles y comas como separadores decimales
                    sesiones_df[col] = sesiones_df[col].astype(str).str.replace('.', '')
                ...
                st.warning(f"La columna {col} no existe en el DataFrame")
        
        # Crear columna con la suma de las zonas de velocidad alta (convertir a km)
        sesiones_df['Alta Velocidad (km)'] = sesiones_df[columnas_velocidad].sum(axis=1) / 1000
        
        # Cambiar: ahora vamos a agrupar por fecha y Session Title para mantener el orden cronológico
        velocidad_stats = sesiones_df.groupby(['Date', 'Session Title'])['Alta Velocidad (km)'].agg(
...
def main():
    ...
    if df is not None:
        ...
        if sesiones_stats is not None and not sesiones_stats.empty:
                ...
                try:
                    if ultima_sesion is not None and not ultima_sesion.empty:
                        ...
                        if all(col in ultima_sesion.columns for col in columnas_velocidad):
                            ...
                            for col in columnas_velocidad:
                                if pd.api.types.is_object_dtype(ultima_sesion[col]):
                                    # Primero eliminar los puntos como separadores de miles
                                    ultima_sesion[col] = ultima_sesion[col].astype(str).str.replace('.', '')
                                    # Luego reemplazar comas por puntos para decimales
                            ...
                            # No sobrescribimos la columna ni convertimos a km
                            # Top 3 por velocidad alta directamente con Sprint Distance (m)
                            top_velocidad = ultima_sesion.sort_values('Sprint Distance (m)', ascending=False).head(3)
                            
                            # Si existe Player Name, es mejor usarla directamente
                            if 'Player Name' in ultima_sesion.columns:
                                top_velocidad_tabla = top_velocidad[['Player Name', 'Sprint Distance (m)']]
                                top_velocidad_tabla['Sprint Distance (m)'] = top_velocidad_tabla['Sprint Distance (m)'].round(2)
                                top_velocidad_tabla.columns = ['Nombre', 'Sprint Distance (m)']
                            else:
                                # Si no existe Player Name, usar Given Name y Family Name
                                top_velocidad_tabla = top_velocidad[['Given Name', 'Family Name', 'Sprint Distance (m)']]
                                top_velocidad_tabla['Sprint Distance (m)'] = top_velocidad_tabla['Sprint Distance (m)'].round(2)
                                top_velocidad_tabla.columns = ['Nombre', 'Apellido', 'Sprint Distance (m)']
                            
                            st.table(top_velocidad_tabla)
                        else:
                            st.warning("No se encontraron datos de Sprint Distance")
                    else:
                        st.warning("No hay datos de la última sesión")
                except Exception as e:
                    import traceback
            ...
            # Top 3 Aceleraciones (corregido para estar al mismo nivel que col_acel_graf)
            with col_acel_tabla:
